...
def function2():
    ...
    # Hint: dtype, reshape    
    newarr = np.arange(10,37).reshape(3, 3, 3)    #wrtie your code here
...
def function10():
    ...
    brr = np.array([1,2,3])
	sub = np.append([arr[0]-brr[0]],[arr[1]-brr[1]], axis=0)
    subt= np.append(sub,[arr[2]-brr[2]],axis=0)# write your code here
...
def function12():
	...
	arr2 = np.tile(arr,3)
    ans = np.concatenate([arr1, arr2])#write your code here 
...
def function13():
    ...
    ans = arr[filter_arr]#write your code here  
    return ans
...
def function14():
    ...
    ans = np.split(arr1,4)#write your code here 
	...
	for a in students:
      for b in piaic:
        if a==b:
          c.append(True)  
    c = np.array(c)
    x = np.count_nonzero(c)# Write you code Here
    return x
